    ...
    <div className="p-6 space-y-6">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Skill Gap Analyzer</h2>
            <p className="text-gray-500">Discover your strengths and areas for improvement</p>
          </div>
        </div>
        ...
        {analysisType === 'upload' ? (
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-400 transition-colors">
            ...
            <p className="text-gray-500 mb-4">Support for PDF, DOC, and DOCX files</p>
            <input
              type="file"
              accept=".pdf,.doc,.docx"
              className="hidden"
      ...
      {analysis && (
        <div className="space-y-6">
          ...
          {/* Overall Score */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Overall Skill Score</h3>
            <div className="flex items-center space-x-6">
              <div className="relative w-24 h-24">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-xl font-bold text-gray-800">{analysis.overallScore || 75}%</span>
                </div>
              </div>
              <div>
                <p className="text-gray-600">Your current skill level positions you well for entry to mid-level roles.</p>
                <p className="text-sm text-gray-500 mt-2">Focus on the recommended skills to reach senior level.</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {analysis.strengths && analysis.strengths.length > 0 ? (
              ...
              <p className="text-gray-500">Strong foundation in frontend technologies and problem-solving.</p>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {analysis.weaknesses && analysis.weaknesses.length > 0 ? (
              ...
              <p className="text-gray-500">Focus on backend development and system architecture skills.</p>
            )}
          </div>
            ...
            {analysis.recommendations && analysis.recommendations.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {analysis.recommendations.map((rec: any, index: number) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    <h4 className="font-medium text-gray-800 mb-2">{rec.skill}</h4>
                    ...
                    <div className="space-y-1">
                      {rec.resources && rec.resources.map((resource: string, idx: number) => (
                        <p key={idx} className="text-sm text-blue-600 hover:underline cursor-pointer">
                          • {resource}
                        </p>
                      ))}
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border border-gray-200 rounded-lg p-4">
                  <h4 className="font-medium text-gray-800 mb-2">Node.js</h4>
                  <span className="inline-block px-2 py-1 rounded-full text-xs font-medium mb-3 bg-red-100 text-red-700">
                    High Priority
                  </span>
                  <div className="space-y-1">
                    <p className="text-sm text-blue-600 hover:underline cursor-pointer">• FreeCodeCamp</p>
                    <p className="text-sm text-blue-600 hover:underline cursor-pointer">• Node.js Documentation</p>
                  </div>
                </div>
                <div className="border border-gray-200 rounded-lg p-4">
                  <h4 className="font-medium text-gray-800 mb-2">MongoDB</h4>
                  <span className="inline-block px-2 py-1 rounded-full text-xs font-medium mb-3 bg-yellow-100 text-yellow-700">
                    Medium Priority
                  </span>
                  <div className="space-y-1">
                    <p className="text-sm text-blue-600 hover:underline cursor-pointer">• MongoDB University</p>
                    <p className="text-sm text-blue-600 hover:underline cursor-pointer">• YouTube Tutorials</p>
                  </div>
                </div>
              </div>
            )}
